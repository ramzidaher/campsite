-- Add additional leave kinds handled through the same approval workflow.
...
alter table public.leave_requests
  ...
  check (kind in ('annual', 'toil', 'parental', 'bereavement', 'compassionate', 'study', 'unpaid'));

alter table public.leave_requests
  drop constraint if exists leave_requests_proposed_kind_check;

alter table public.leave_requests
  add constraint leave_requests_proposed_kind_check
  check (
    proposed_kind is null
    or proposed_kind in ('annual', 'toil', 'parental', 'bereavement', 'compassionate', 'study', 'unpaid')
  );
