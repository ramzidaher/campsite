-- main_shell_layout_structural: one profiles + auth.users + organisations read
-- instead of two profiles scans plus a separate auth.users lookup.
-- job_applications: index aligned with org + job listing lists ordered by submitted_at.
...
create index if not exists job_applications_org_job_submitted_at_desc_idx
  on public.job_applications (org_id, job_listing_id, submitted_at desc);
