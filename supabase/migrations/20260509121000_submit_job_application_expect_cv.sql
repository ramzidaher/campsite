-- Allow submit_job_application to accept p_expect_cv_upload when a CV file will be
-- attached immediately after insert via service role (path {org_id}/{application_id}/...).

drop function if exists public.submit_job_application(text, text, text, text, text, text, text, smallint);
