-- Open-slot booking improvements:
-- 1) user opt-in flag for open-slot alerts
-- 2) notification recipient fan-out for open slots
-- 3) include dept_id in notification payload for scoped recipient matching

alter table public.profiles
  add column if not exists rota_open_slot_alerts_enabled boolean not null default false;

comment on column public.profiles.rota_open_slot_alerts_enabled is
  'When true, user receives notifications about newly created open rota slots they can claim.';
...
begin
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'dept_id', case when tg_op = 'DELETE' then old.dept_id else new.dept_id end
    )
  );
...
begin
  ...
  if j.event_type = 'shift_reminder' then
    assignee := null;
  ...
  if j.event_type in ('shift_created', 'shift_updated', 'shift_deleted') then
    ...
    did := null;
    if j.payload ? 'dept_id' and j.payload->'dept_id' is not null and jsonb_typeof(j.payload->'dept_id') <> 'null' then
      begin
        did := (j.payload->>'dept_id')::uuid;
      exception when others then
        did := null;
      end;
    end if;
    ...
    from (
      ...
      where j.event_type in ('shift_created', 'shift_updated')
        and assignee is null
        and p.org_id = j.org_id
        and p.status = 'active'
        and p.rota_open_slot_alerts_enabled = true
        and (
          did is null
          or exists (
