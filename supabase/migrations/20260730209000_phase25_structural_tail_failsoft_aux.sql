-- Phase 2.5 surgical fix:
-- Cap rare long-tail stalls inside main_shell_layout_structural by fail-softing
-- non-critical auxiliary lookups (dept + celebration overrides).

create or replace function public._safe_dept_name_text(p_user_id uuid, p_timeout_ms integer default 400)
returns text
language plpgsql
stable
security definer
...
begin
  ...
  perform set_config('statement_timeout', greatest(p_timeout_ms, 50)::text, true);
  ...
  perform set_config('statement_timeout', coalesce(v_prev_timeout, '0'), true);
  return v_dept_name;
exception
...
create or replace function public._safe_org_celebration_modes_json(p_org_id uuid, p_timeout_ms integer default 400)
returns jsonb
language plpgsql
stable
security definer
