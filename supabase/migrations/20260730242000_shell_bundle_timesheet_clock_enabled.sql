-- Expose `timesheet_clock_enabled` from employee_hr_records in shell bundles so the
-- app can show Attendance nav only for members who use the clock (no extra round trip).
