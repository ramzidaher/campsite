-- Leave carry-over workflow: request-by-request approvals with per-org max cap.

alter table public.org_leave_settings
  add column if not exists carry_over_enabled boolean not null default false,
  add column if not exists carry_over_requires_approval boolean not null default true,
  add column if not exists carry_over_max_days numeric(10, 2) not null default 0
    check (carry_over_max_days >= 0 and carry_over_max_days <= 365);

comment on column public.org_leave_settings.carry_over_enabled is
  'When true, employees can request annual leave carry-over from one leave year into the next.';
comment on column public.org_leave_settings.carry_over_requires_approval is
  'When true, each carry-over request requires manager/admin approval.';
comment on column public.org_leave_settings.carry_over_max_days is
  'Maximum annual leave days that can be carried into next leave year per request.';

create table if not exists public.leave_carryover_requests (
  ...
  unique (org_id, requester_id, from_leave_year, to_leave_year, created_at)
);

create index if not exists leave_carryover_requests_org_status_idx
  on public.leave_carryover_requests (org_id, status);
create index if not exists leave_carryover_requests_requester_idx
  on public.leave_carryover_requests (requester_id, created_at desc);

alter table public.leave_carryover_requests enable row level security;

drop policy if exists leave_carryover_requests_select on public.leave_carryover_requests;
create policy leave_carryover_requests_select
    ...
    and (
      ...
      or (
        ...
        and exists (
          ...
          where s.id = leave_carryover_requests.requester_id
...
create or replace function public.leave_carryover_available_days(
  p_org_id uuid,
  p_user_id uuid,
  p_from_leave_year text
)
returns numeric
language plpgsql
stable
...
begin
  ...
  where la.org_id = p_org_id
    ...
    and la.leave_year = p_from_leave_year;
  ...
  where c.org_id = p_org_id
    ...
    and c.from_leave_year = p_from_leave_year
    and c.status in ('pending', 'approved');

  return greatest(0, coalesce(v_ent, 0) - coalesce(v_used, 0) - coalesce(v_already_requested, 0));
end;
$$;

revoke all on function public.leave_carryover_available_days(uuid, uuid, text) from public;

create or replace function public.leave_carryover_can_decide_request(p_viewer uuid, p_request_id uuid)
returns boolean
language plpgsql
stable
security definer
...
declare
  ...
  v_requester uuid;
begin
  select c.org_id, c.requester_id
...
revoke all on function public.leave_carryover_can_decide_request(uuid, uuid) from public;

create or replace function public.leave_carryover_request_submit(
  p_from_leave_year text,
...
begin
  ...
  if p_from_leave_year is null or trim(p_from_leave_year) !~ '^[0-9]{4}$' then
    raise exception 'invalid from_leave_year';
  end if;
  ...
  select
    coalesce(ols.carry_over_enabled, false),
    coalesce(ols.carry_over_requires_approval, true),
    coalesce(ols.carry_over_max_days, 0)
  into v_enabled, v_requires_approval, v_max
  from public.org_leave_settings ols
  where ols.org_id = v_org;

  if not coalesce(v_enabled, false) then
    raise exception 'carry-over is disabled for this organisation';
  end if;

  v_to_leave_year := ((trim(p_from_leave_year))::int + 1)::text;
  v_available := public.leave_carryover_available_days(v_org, v_uid, trim(p_from_leave_year));
  v_req := least(p_days_requested, coalesce(v_max, 0), v_available);

  if v_req <= 0 then
    raise exception 'no carry-over days available to request';
  end if;

  if v_requires_approval then
    insert into public.leave_carryover_requests (
      org_id, requester_id, from_leave_year, to_leave_year, days_requested, note, status
    )
    values (
      v_org, v_uid, trim(p_from_leave_year), v_to_leave_year, round(v_req, 2), nullif(trim(coalesce(p_note, '')), ''), 'pending'
    )
  ...
  v_apply := round(v_req, 2);
  perform public.leave_ensure_allowance_row(v_org, v_uid, v_to_leave_year);

  update public.leave_allowances
  set
    annual_entitlement_days = annual_entitlement_days + v_apply,
    updated_at = now()
  where org_id = v_org and user_id = v_uid and leave_year = v_to_leave_year;

  insert into public.leave_carryover_requests (
    org_id, requester_id, from_leave_year, to_leave_year, days_requested, days_approved, note, status, decided_by, decided_at
  )
  values (
    v_org, v_uid, trim(p_from_leave_year), v_to_leave_year, v_apply, v_apply, nullif(trim(coalesce(p_note, '')), ''), 'approved', v_uid, now()
  )
  returning id into rid;
...
revoke all on function public.leave_carryover_request_submit(text, numeric, text) from public;
grant execute on function public.leave_carryover_request_submit(text, numeric, text) to authenticated;

create or replace function public.leave_carryover_request_decide(
  p_request_id uuid,
  p_approve boolean,
  p_note text
)
...
begin
  ...
  if not public.leave_carryover_can_decide_request(v_uid, p_request_id) then
    raise exception 'not allowed';
  end if;

  select c.org_id, c.requester_id, c.from_leave_year, c.to_leave_year, c.days_requested, c.status
  into v_org, v_requester, v_from, v_to, v_req, v_status
  from public.leave_carryover_requests c
  where c.id = p_request_id;
  ...
  select coalesce(ols.carry_over_max_days, 0)
  into v_max
  from public.org_leave_settings ols
  where ols.org_id = v_org;

  v_available := public.leave_carryover_available_days(v_org, v_requester, v_from);
  v_apply := round(least(v_req, coalesce(v_max, 0), v_available), 2);

  if v_apply <= 0 then
    raise exception 'no carry-over days available to approve';
  end if;

  perform public.leave_ensure_allowance_row(v_org, v_requester, v_to);

  update public.leave_allowances
  set
    annual_entitlement_days = annual_entitlement_days + v_apply,
    updated_at = now()
  where org_id = v_org and user_id = v_requester and leave_year = v_to;
...
revoke all on function public.leave_carryover_request_decide(uuid, boolean, text) from public;
grant execute on function public.leave_carryover_request_decide(uuid, boolean, text) to authenticated;
...
begin
  ...
  if public.has_permission(v_uid, v_org, 'leave.manage_org', '{}'::jsonb) then
    ...
    return coalesce(n, 0) + coalesce(m, 0) + coalesce(c, 0);
  end if;
