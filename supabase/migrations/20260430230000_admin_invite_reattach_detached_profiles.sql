-- Re-inviting a user after org_admin_remove_member: profile stays with org_id null.
-- Old logic used "v_existing_org is distinct from p_org_id", which is TRUE for NULL vs uuid
-- and wrongly raised "already belongs to another organisation".
-- Also set org_id on UPDATE so detached members are reattached to the inviting org.
...
comment on function public.admin_provision_invited_member(uuid, uuid, text, text, uuid[]) is
  'Creates or updates profiles in org after invite / existing-auth add; service_role only. org_id null (detached) can rejoin the inviting org.';
