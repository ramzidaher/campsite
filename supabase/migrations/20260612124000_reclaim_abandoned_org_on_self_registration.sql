-- After deleting the only auth user for a self-created tenant, the organisation row remains
-- (profiles.id references auth.users ON DELETE CASCADE). Re-signup with the same workspace slug
-- used to fail with "slug taken" / generic database error. Allow reclaim when the org has no members.
