...
begin
  ...
  v_founder_name := nullif(trim(coalesce(p_meta->>'register_founder_org_name', '')), '');
  v_founder_slug_raw := nullif(trim(coalesce(p_meta->>'register_founder_org_slug', '')), '');
  ...
  if v_org is not null and v_founder_name is not null and v_founder_slug_raw is not null then
    raise exception 'Invalid registration: choose either joining an organisation or creating one, not both';
  end if;

  -- Founder path: new organisation + active org_admin
  if v_founder_name is not null and v_founder_slug_raw is not null then
    if length(v_founder_name) > 120 or length(v_founder_name) < 1 then
      raise exception 'Organisation name must be between 1 and 120 characters';
    end if;

    v_slug := lower(v_founder_slug_raw);
    ...
    values (v_founder_name, v_slug, true)
