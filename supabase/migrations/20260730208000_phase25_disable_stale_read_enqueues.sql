-- Phase 2.5 emergency hardening:
-- Remove stale-read enqueue writes from hot shell RPC reads.
-- These writes can amplify contention under burst traffic.
-- We still enqueue on cache miss and rely on write-event triggers + cron drain.
