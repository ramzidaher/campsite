-- Live org chart status foundation:
-- - presence tracking via profiles.last_seen_at
-- - heartbeat RPC for authenticated user
-- - org-scoped live org chart RPC (hierarchy + shift/approval/activity status)

alter table public.profiles
  add column if not exists last_seen_at timestamptz;

comment on column public.profiles.last_seen_at is
  'Latest activity heartbeat timestamp for live presence surfaces (e.g. org chart).';

create index if not exists profiles_org_last_seen_idx
  on public.profiles (org_id, last_seen_at desc)
  where status = 'active';
...
begin
  ...
  v_can_view_all :=
    public.has_permission(v_uid, v_org, 'reports.view', '{}'::jsonb)
