...
export async function GET() {
  ...
  const { data: canExport } = await supabase.rpc('has_permission', {
    p_user_id: user.id, p_org_id: orgId, p_permission_key: 'payroll.uk_tax.export', p_context: {},
  });
  if (!canExport) return NextResponse.json({ error: 'Not allowed' }, { status: 403 });

  const { data: rows, error } = await supabase
    .from('employee_uk_tax_details')
    .select('id, user_id, encrypted_payload, effective_from')
    .eq('org_id', orgId)
    .eq('is_active', true)
    .eq('status', 'approved')
  ...
  const lines: string[] = [];
  lines.push(['employee_id', 'employee_name', 'employee_email', 'ni_number', 'tax_code', 'starter_declaration', 'student_loan_plan', 'postgraduate_loan', 'tax_basis', 'notes', 'effective_from'].join(','));

  for (const row of rows ?? []) {
    const p = profileById.get(row.user_id as string);
    const d = decryptUkTaxDetails(row.encrypted_payload as string);
    ...
    lines.push([
      ...
      csvEscape(d.ni_number || ''),
      csvEscape(d.tax_code || ''),
      csvEscape(d.starter_declaration || ''),
      csvEscape(d.student_loan_plan || ''),
      csvEscape(String(d.postgraduate_loan || false)),
      csvEscape(d.tax_basis || ''),
      csvEscape(d.notes || ''),
  ...
  return new NextResponse(lines.join('\n'), {
    ...
    headers: {
      ...
      'Content-Disposition': `attachment; filename="payroll-uk-tax-${new Date().toISOString().slice(0, 10)}.csv"`,
