...
export async function GET(request: NextRequest) {
  ...
  const [tsRes, wagesRes, adjustmentsRes] = await Promise.all([tsQuery, wagesQuery, adjustmentQuery]);
  ...
  const { data: hrPayProfiles } = ids.length
    ? await supabase.from('employee_hr_records').select('user_id, pay_frequency').eq('org_id', orgId).in('user_id', ids)
  ...
  const dataRows = timesheets.map((t) => {
    ...
    return {
      ...
      pay_frequency: payFrequencyById.get(t.user_id) ?? 'weekly',
