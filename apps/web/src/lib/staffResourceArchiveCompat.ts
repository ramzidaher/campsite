...
/**
 * True when PostgREST/Postgres errors indicate `staff_resources.archived_at` is missing
 * (migration not applied on remote) or not exposed in schema cache.
 */
...
export const STAFF_RESOURCE_DETAIL_SELECT_LEGACY =
...
/** Load a resource row for API routes; retries without `archived_at` if the column is missing. */
