...
export async function loadWorkspaceDepartmentIds(
  ...
  const r = role?.trim();
  if (r === 'manager') {
    const { data } = await supabase.from('dept_managers').select('dept_id').eq('user_id', userId);
    return [...new Set((data ?? []).map((row) => row.dept_id as string))];
  }
  if (r === 'coordinator') {
    const { data } = await supabase.from('user_departments').select('dept_id').eq('user_id', userId);
    return [...new Set((data ?? []).map((row) => row.dept_id as string))];
  }
  return [];
}
