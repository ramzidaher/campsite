...
/**
 * Minute span for highlighting a shift on its **start** calendar day - matches `layoutWeekShifts` / `clampToGrid`
...
/** Pixel box for a draft / highlight range (local minutes from midnight, same day). */
...
/** Snap total minutes-from-midnight to grid + SNAP_MINUTES. */
...
/** Build new ISO range for a shift moved to another day (weekDays index) and new start minute-of-day (local). Preserves duration in ms. */
...
function wallClockOnDay(day: Date, minutesFromMidnight: number): Date {
...
/** Drag top edge: new start time (snapped), end unchanged. Null if invalid (after end or too short). */
export function snappedResizeShiftStart(
  shift: { start_time: string; end_time: string },
  weekDay: Date,
  newStartMinutesFromMidnight: number,
): { start_time: string; end_time: string } | null {
  const snapped = snapMinutesFromMidnight(newStartMinutesFromMidnight);
  const start = wallClockOnDay(weekDay, snapped);
  const end = new Date(shift.end_time);
  if (start.getTime() >= end.getTime()) return null;
  const minMs = SNAP_MINUTES * 60 * 1000;
  if (end.getTime() - start.getTime() < minMs) return null;
  return { start_time: start.toISOString(), end_time: end.toISOString() };
}

/** Drag bottom edge: new end time (snapped), start unchanged. If same-day wall time would be before start, uses next day. */
export function snappedResizeShiftEnd(
  shift: { start_time: string; end_time: string },
  weekDay: Date,
  newEndMinutesFromMidnight: number,
): { start_time: string; end_time: string } | null {
  const snapped = snapMinutesFromMidnight(newEndMinutesFromMidnight);
  const start = new Date(shift.start_time);
  let end = wallClockOnDay(weekDay, snapped);
  if (end.getTime() <= start.getTime()) {
    end = new Date(end);
