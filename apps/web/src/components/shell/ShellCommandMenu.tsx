...
function HighlightText({ text, query }: { text: string; query: string }) {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <strong className="font-semibold text-[#121212]">{text.slice(idx, idx + q.length)}</strong>
  ...
  const palette = open ? (
    ...
    >
      <div
        ...
        className="flex max-h-[min(72vh,640px)] w-full max-w-[min(100vw-1.5rem,520px)] flex-col overflow-hidden rounded-2xl border border-[#e4e2de] bg-white shadow-[0_24px_80px_rgba(0,0,0,0.18)]"
      ...
      >
        ...
        <div className="flex items-center gap-3 border-b border-[#eceae6] px-4 py-3">
          <Search className="h-[18px] w-[18px] shrink-0 text-[#9b9b9b]" strokeWidth={2} aria-hidden />
          <input
            ref={inputRef}
            type="search"
            value={q}
            ...
            className="min-w-0 flex-1 border-0 bg-transparent text-[15px] text-[#121212] outline-none placeholder:text-[#9b9b9b]"
            autoComplete="off"
            autoCorrect="off"
            spellCheck={false}
          />
        ...
        <div ref={listRef} className="min-h-0 flex-1 overflow-y-auto px-1.5 py-2">
          {flatRows.length === 0 ? (
            <p className="px-3 py-6 text-center text-[13px] text-[#6b6b6b]">No matching pages or actions.</p>
          ) : (
            flatRows.map((row, i) => {
              if (row.type === 'heading') {
                return (
                  <div
                    key={row.key}
                    className="px-2.5 pb-1 pt-2 text-[10px] font-semibold uppercase tracking-[0.1em] text-[#9b9b9b]"
                  >
                    {row.text}
                  </div>
                );
              ...
              return (
                <button
                  ...
                  className={[
                    ...
                    isDead ? 'cursor-default text-[#6b6b6b]' : 'text-[#121212]',
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    {row.row?.sub ? (
                      <span className="mt-0.5 block truncate text-[11.5px] text-[#6b6b6b]">{row.row.sub}</span>
                    ) : null}
                  </span>
                </button>
              );
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 border-t border-[#eceae6] px-4 py-2.5 text-[11px] text-[#9b9b9b]">
          <span className="inline-flex items-center gap-1.5">
            <span className="rounded-md border border-[#e4e2de] bg-[#faf9f6] px-1.5 py-0.5 font-medium text-[#6b6b6b]">
              ↑↓
            </span>
            navigate
            <span className="rounded-md border border-[#e4e2de] bg-[#faf9f6] px-1.5 py-0.5 font-medium text-[#6b6b6b]">
          ...
          <span className="max-w-[55%] truncate rounded-full border border-[#eceae6] bg-[#faf9f6] px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-[0.06em] text-[#6b6b6b]">
            Org · {orgName}
          </span>
        </div>
      </div>
  ...
  return (
    <>
      <button
        ...
        className="group flex h-9 w-full min-w-0 max-w-[min(100%,420px)] items-center gap-2.5 rounded-full border border-[#d8d8d8] bg-white px-3.5 text-left shadow-[0_1px_2px_rgba(0,0,0,0.04)] transition-[border-color,box-shadow] hover:border-[#c5c5c5] hover:shadow-[0_2px_8px_rgba(0,0,0,0.06)]"
        aria-haspopup="dialog"
        aria-expanded={open}
      >
        <Search className="h-4 w-4 shrink-0 text-[#9b9b9b]" strokeWidth={2} aria-hidden />
        <span className="min-w-0 flex-1 truncate text-[13px] text-[#9b9b9b]">Search…</span>
        <kbd
          className="hidden shrink-0 rounded-md border border-[#e8e6e3] bg-[#f5f4f1] px-1.5 py-0.5 font-sans text-[10px] font-medium text-[#6b6b6b] sm:inline-block"
