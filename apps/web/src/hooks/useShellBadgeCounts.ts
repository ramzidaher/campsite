...
function parseCounts(data: unknown): ShellBadgeCounts {
 ...
 *
 * - Polls every 60 s in the background (replaces router.refresh() every 3 s)
 * - Immediately refetches on window focus / tab visibility
 * - Initial values come from the server-rendered layout (no flash of zeros)
 */
